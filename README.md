...
- **LM Studio:** load a downloaded model, open **Developer**, and start the local server (default `127.0.0.1:1234`). Select **LM Studio · local**, refresh diagnostics, and use the model ID shown there. The detected local Qwen model uses `qwen/qwen3.8-27b`; it must be loaded or visible to the server first. If you prefer the terminal, run `lms load qwen/qwen3.8-27b`, then `lms server start`. Codex CLI and account authentication are unnecessary.
